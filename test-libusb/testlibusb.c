...
static int print_device(libusb_device *dev, int level)
{
	...
	//fprintf(stderr, "libusb_get_device_descriptor(0x%lx, &desc)\n", dev);
	ret = libusb_get_device_descriptor(dev, &desc);
	if (ret < 0) {
		fprintf(stderr, "failed to get device descriptor");
		return -1;
	}
	//fprintf(stderr, "libusb_get_device_descriptor(0x%lx, &desc) := %d\n", dev, ret);

	ret = libusb_open(dev, &handle);
	//fprintf(stderr, "libusb_open(dev, &handle) := %d\n", ret);

	if (LIBUSB_SUCCESS == ret) {
		fprintf(stdout, "LIBUSB_SUCCESS \n");
	...
	fprintf(stdout, "%.*sDev (bus %d, device %d): %s\n", level * 2, "                    ",
...
int main(int argc, char *argv[])
{
	...
	fprintf(stdout, "cnt := %d, devs := 0x%lx.\n", cnt, devs);

	if (cnt < 0)
		return (int)cnt;

	for (i = 0; devs[i]; ++i) {
		//fprintf(stderr, "print_device(devs[%d] := 0x%lx, 0);\n", i, devs[i]);
