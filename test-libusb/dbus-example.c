...
int
callMethod(char *method, char *_param, char *_param1)
{
	DBusMessage* msg;
	DBusMessageIter args;
	DBusPendingCall* pending;
	...
	if (_param1){
	dbus_uint64_t param1 = atol(_param1);
	if(!dbus_message_iter_append_basic(&args, DBUS_TYPE_UINT64, &param1)) { 
		fprintf(stderr, "Out Of Memory!\n"); 
		exit(1);
	}
	}
	...
	dbus_uint64_t varuint64;
	dbus_uint32_t varuint32;
	char varstring[1024];

	dbus_uint64_t level = 0;
	...
	if (dbus_message_iter_init(msg, &args)){
		...
		do {
			fprintf(stderr, "(%c) ", dbus_message_iter_get_arg_type(&args)); 
			switch(dbus_message_iter_get_arg_type(&args)){
				case 'i':
		  			dbus_message_iter_get_basic(&args, &varuint32);
					fprintf(stderr, "%d, ", varuint32); 
					break;
				case 't':
		  			dbus_message_iter_get_basic(&args, &varuint64);
					fprintf(stderr, "0x%lx(%ld), ", varuint64, varuint64); 
					break;
				case 's':
		  			dbus_message_iter_get_basic(&args, varstring);
					fprintf(stderr, "%s, ", varstring);
					break;
			}//endswitch
...
int main(int argc, char** argv)
{
	...
	char* param = "no param", *param1;
	char *method;
	if (argc >= 3 && NULL != argv[2]) method = argv[2];
	if (argc >= 4 && NULL != argv[3]) param = argv[3];
	if (argc >= 5 && NULL != argv[3]) param1 = argv[4]; else param1 = NULL;
	...
	if (0 == strcmp(argv[1], "send"))
		...
		callMethod(method, param, param1);
	else {
		printf ("Syntax: dbus-example [send|receive|listen|query] [<param>]\n");
		return 1;
	}
